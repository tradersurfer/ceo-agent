...
class Organization {
  ...
  findDepartment(id) { return this.departments.get(id) || null; }
  findRole(id) { return this.roles.get(id) || null; }
  findAgent(id) { return this.agents.get(id) || null; }
  listDepartments() { return [...this.departments.values()]; }
  ...
  getOrganizationChart() {
    return {
      ...
      departments: this.listDepartments().map(department => ({
        ...
        managerRole: department.managerRole, status: department.status,
        roles: department.listRoles().map(id => {
          const role = this.findRole(id);
          return role ? { id: role.id, title: role.title, assignedAgent: role.assignedAgent, filled: role.isFilled() } : { id };
        }),
  ...
  exportBlueprint() {
    return {
      organizationId: this.organizationId, organizationName: this.organizationName, owner: this.owner,
      runtime: this.runtime, created: this.created, status: this.status,
      departments: this.listDepartments().map(department => ({
        id: department.id, name: department.name, description: department.description,
        managerRole: department.managerRole, roles: department.listRoles(), agents: department.listAgents(), status: department.status,
      })),
      roles: this.listRoles().map(role => ({
        id: role.id, title: role.title, department: role.department, description: role.description,
        requiredCapabilities: [...role.requiredCapabilities], assignedAgent: role.assignedAgent,
      })),
      agents: this.listAgents().map(agent => agent.getSummary()),
    };
  }
  static createDefault() {
    const organization = new Organization({ organizationId: 'business_org', organizationName: null, owner: null, runtime: 'CEO Agent Runtime' });
    for (const [id, name] of [['executive','Executive Office'],['finance','Finance'],['operations','Operations'],['technology','Technology'],['marketing','Marketing'],['people','People'],['legal','Legal & Compliance']]) {
      organization.registerDepartment({ id, name });
    }
    const roles = [
