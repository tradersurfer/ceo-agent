...
/**
 * Builds the resolved role -> {flagship, efficient} catalog from a live
 * ModelBroker. Always OpenRouter's catalog only (ModelBroker has no other
 * source in Phase 1) — never a stand-in for a direct provider's model list.
 * @param {import('../core/ModelBroker')} modelBroker
 * @returns {object} Map of role -> { flagship, efficient }, entries null until resolved.
 */
function buildCatalog(modelBroker) {
  ...
  for (const role of CHAT_ROLES) {
    ...
    catalog[role] = model && model.tiers
      ? { flagship: model.tiers.flagship || null, efficient: model.tiers.efficient || null }
      : { flagship: null, efficient: null };
  }
  return catalog;
}
